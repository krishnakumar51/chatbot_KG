...
from qa.open import get_qa_response, initialize_system, create_chain_with_retrieval
import os
...
if "initialized" not in st.session_state:
    # Load the LLM, Neo4j graph, and embeddings vector index
    st.session_state.llm, st.session_state.graph, st.session_state.vector_index = initialize_system()
    # Create the chain combining retrieval and entity extraction with Neo4j and LLM
    st.session_state.chain = create_chain_with_retrieval(st.session_state.llm, st.session_state.graph, st.session_state.vector_index)
