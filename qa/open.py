...
def create_chain(llm):
    """
    Create the chain that extracts entities without bind_parser() issues.
    """

    # Step 1: Define the system prompt for entity extraction
    entity_prompt = ChatPromptTemplate.from_messages([
        ("system", 
        "You are a concise assistant designed to extract entities from text. "
        "Return the result as a JSON object with the format: {\"names\": [\"Entity1\", \"Entity2\"]}."),
        ("human", "{input}")
    ])

    # Step 2: Function to process and validate the response
    def extract_entities(question: str):
        prompt_input = entity_prompt.format_messages(input=question)
        try:
            # Invoke the LLM
            response = llm(prompt_input)
            # Extract content from the LLM's raw response (assuming it returns text)
            raw_content = response["choices"][0]["message"]["content"]
            # Try parsing the JSON string in the content
            parsed_response = json.loads(raw_content)
            # Validate the parsed response using Pydantic
            entities = Entities(**parsed_response)
...
def create_chain_with_retrieval(llm, graph, vector_index):
    """
    ...
    # Step 1: Create entity extraction chain using the LLM with structured output mode
    extract_entities = create_chain(llm)

    # Step 2: Set up the retriever to fetch relevant documents from the Neo4j vector index
    retriever = vector_index.as_retriever()

    # Step 3: Create the full retrieval-augmented generation (RAG) chain
    combine_docs_chain = create_stuff_documents_chain(llm)

    rag_chain = create_retrieval_chain(retriever, combine_docs_chain)

    # Step 4: Function to handle chat history and query for RAG chain
    def get_condensed_query_with_history(chat_history, question):
        """
    ...
    # Step 5: Structured retriever for retrieving data from the graph using entities
    ...
    # Step 6: General retriever combining structured and unstructured data
